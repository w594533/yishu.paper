        ...
        <div class="product-img-teams-left-text">
          <ul>
            @foreach ($product_categories as $product_category)
              <li><span class="@if ($product_category->id == $parent_product_category_id) active @endif">{{ $product_category->title }}</span></li>
              @endforeach
            </ul>
          </div>
        </div>
          ...
          <div class="sub-category">
            <ul>
              @foreach ($products->childrenCategories as $product)
                <li class="@if ($loop->first) active @endif">{{ $product->title }}</li>
                @endforeach
              </ul>
            </div>
            @if (count($images) >0)
